...
fn test_regime_failures_escalate_appropriately() {
    ...
    let severe_pattern = new_patterns
        .iter()
        .find(|p| p.category == FailureCategory::RegimeMisclassification)
        .unwrap();
...
fn test_learning_report_identifies_problem_components() {
    ...
    assert!(report
        ...
        .contains(&"problematic_signal".to_string()));

    // Should identify broken component for deprecation
    assert!(report
        .deprecation_candidates
        .contains(&"broken_model".to_string()));

    // Stable component should not appear in either list
    assert!(!report
        .components_needing_attention
        .contains(&"stable_component".to_string()));
    assert!(!report
        .deprecation_candidates
        .contains(&"stable_component".to_string()));
}
...
fn test_failure_categories_have_correct_response_strategies() {
    ...
    assert!(FailureCategory::DataFailure
        .response_strategy()
        .contains("do not adjust inference logic"));

    assert!(FailureCategory::ModelAssumptionFailure
        .response_strategy()
        .contains("model revision"));

    assert!(FailureCategory::RegimeMisclassification
        .response_strategy()
        .contains("regime detection"));

    assert!(FailureCategory::SignalMisapplication
        .response_strategy()
        .contains("applicability rules"));

    assert!(FailureCategory::OverconfidenceFailure
        .response_strategy()
