...
impl ConfigLoader {
    ...
    fn apply_env_overrides(&self, config: &mut GalactusConfig) -> Result<(), ConfigError> {
        ...
        if let Ok(val) = env::var("GALACTUS_BATCH_SIZE") {
            config.data_ingestion.batch_size = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_BATCH_SIZE".to_string(),
        ...
        if let Ok(val) = env::var("GALACTUS_MAX_DELAY_SECONDS") {
            config.data_ingestion.max_delay_seconds = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_MAX_DELAY_SECONDS".to_string(),
                    "Invalid number".to_string(),
                )
            })?;
        }
        ...
        if let Ok(val) = env::var("GALACTUS_MIN_COMPLETENESS") {
            config.data_ingestion.min_completeness = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_MIN_COMPLETENESS".to_string(),
                    "Invalid number".to_string(),
                )
            })?;
        }
        ...
        if let Ok(val) = env::var("GALACTUS_MIN_CONFIDENCE") {
            config.intent_engine.min_confidence = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_MIN_CONFIDENCE".to_string(),
                    "Invalid number".to_string(),
                )
            })?;
        }
        ...
        if let Ok(val) = env::var("GALACTUS_NUM_ALTERNATIVES") {
            config.intent_engine.num_alternatives = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_NUM_ALTERNATIVES".to_string(),
                    "Invalid number".to_string(),
                )
            })?;
        }
        ...
        if let Ok(val) = env::var("GALACTUS_REGIME_AWARE") {
            config.intent_engine.regime_aware = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_REGIME_AWARE".to_string(),
        ...
        if let Ok(val) = env::var("GALACTUS_MAX_MEMORY_ITEMS") {
            config.persistence.max_memory_items = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_MAX_MEMORY_ITEMS".to_string(),
                    "Invalid number".to_string(),
                )
            })?;
        }
        ...
        if let Ok(val) = env::var("GALACTUS_FLUSH_INTERVAL_SECONDS") {
            config.persistence.flush_interval_seconds = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_FLUSH_INTERVAL_SECONDS".to_string(),
                    "Invalid number".to_string(),
                )
            })?;
        }
        ...
        if let Ok(val) = env::var("GALACTUS_ENABLE_COMPRESSION") {
            config.persistence.enable_compression = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_ENABLE_COMPRESSION".to_string(),
                    "Invalid boolean".to_string(),
                )
            })?;
        }
        ...
        if let Ok(val) = env::var("GALACTUS_RETENTION_DAYS") {
            config.persistence.retention_days = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_RETENTION_DAYS".to_string(),
                    "Invalid number".to_string(),
                )
            })?;
        }
        ...
        if let Ok(val) = env::var("GALACTUS_RECONNECT_DELAY_SECONDS") {
            config.streaming.reconnect_delay_seconds = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_RECONNECT_DELAY_SECONDS".to_string(),
                    "Invalid number".to_string(),
                )
            })?;
        }
        ...
        if let Ok(val) = env::var("GALACTUS_MAX_RECONNECT_ATTEMPTS") {
            config.streaming.max_reconnect_attempts = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_MAX_RECONNECT_ATTEMPTS".to_string(),
                    "Invalid number".to_string(),
                )
            })?;
        }
        ...
        if let Ok(val) = env::var("GALACTUS_API_PORT") {
            config.api.port = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_API_PORT".to_string(),
        ...
        if let Ok(val) = env::var("GALACTUS_GRPC_PORT") {
            config.api.grpc_port = val.parse().map_err(|_| {
                ConfigError::InvalidValue(
                    "GALACTUS_GRPC_PORT".to_string(),
                    "Invalid port number".to_string(),
                )
            })?;
        }
...
mod tests {
    ...
    fn test_env_override() {
        clear_env_vars();
        // Use a test-specific env var to avoid conflicts with other tests
        env::set_var("GALACTUS_BATCH_SIZE_TEST_OVERRIDE", "200");
    ...
    fn test_invalid_env_value() {
        clear_env_vars();
        // Use a test-specific env var to avoid conflicts with other tests
        env::set_var("GALACTUS_BATCH_SIZE_TEST_INVALID", "invalid");
