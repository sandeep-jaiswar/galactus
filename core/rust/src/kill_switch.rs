...
impl KillSwitchDecision {
    ...
    pub fn can_proceed(&self) -> bool {
        matches!(
            self,
            KillSwitchDecision::Proceed | KillSwitchDecision::ProceedWithWarnings(_)
        )
    }
...
impl Default for KillSwitchEvaluator {
    fn default() -> Self {
...
impl KillSwitchEvaluator {
    ...
    fn check_data_integrity(
        ...
        if !data_quality.integrity_valid {
            ...
            context.insert(
                ...
                data_quality.data_completeness.to_string(),
            );
        ...
        if !data_quality.schema_valid {
            conditions.push(KillCondition {
                ...
                description: "Schema validation failed - unresolvable mismatch detected"
                    .to_string(),
                severity: KillSeverity::Mandatory,
                context: HashMap::new(),
            });
        ...
        if data_quality.data_completeness < self.config.min_data_completeness {
            let mut context = HashMap::new();
            context.insert(
                "actual_completeness".to_string(),
                data_quality.data_completeness.to_string(),
            );
            context.insert(
                "required_completeness".to_string(),
                self.config.min_data_completeness.to_string(),
            );
    ...
    fn check_time_semantics(
        ...
        if data_quality.max_time_ambiguity > self.config.max_time_ambiguity_seconds {
            let mut context = HashMap::new();
            context.insert(
                "max_ambiguity".to_string(),
                data_quality.max_time_ambiguity.to_string(),
            );
            context.insert(
                "allowed_ambiguity".to_string(),
                self.config.max_time_ambiguity_seconds.to_string(),
            );
            ...
            conditions.push(KillCondition {
                ...
                description: format!(
                    ...
                    data_quality.max_time_ambiguity, self.config.max_time_ambiguity_seconds
                ),
                severity: KillSeverity::Mandatory,
                context,
            });
    ...
    fn check_regime_validity(
        ...
        if regime.regime_confidence < self.config.min_regime_confidence {
            ...
            context.insert(
                ...
                regime.regime_confidence.to_string(),
            );
            context.insert(
                "required_confidence".to_string(),
                self.config.min_regime_confidence.to_string(),
            );
            ...
            conditions.push(KillCondition {
                ...
                description: format!(
                    ...
                    regime.regime_confidence, self.config.min_regime_confidence
                ),
                severity: KillSeverity::Mandatory,
                context,
            });
        ...
        if regime.has_conflicts {
            ...
            context.insert(
                ...
                regime.conflicts.len().to_string(),
            );
    ...
    fn check_structural_validity(
        ...
        if !structural.constraints_valid {
            ...
            conditions.push(KillCondition {
                ...
                description:
                    "Core constraint assumptions invalidated - structural model inconsistent"
                        .to_string(),
                severity: KillSeverity::Mandatory,
                context,
            });
        ...
        if !structural.forced_flow_consistent {
            conditions.push(KillCondition {
                ...
                description:
                    "Forced flow logic contradicted structurally - cannot maintain consistency"
                        .to_string(),
                severity: KillSeverity::Mandatory,
                context: HashMap::new(),
            });
    ...
    fn check_confidence_system(
        ...
        if !confidence.score.is_finite() || !stability.score.is_finite() {
            conditions.push(KillCondition {
                ...
                description:
                    "Confidence calculation unavailable or corrupted - non-finite values detected"
                        .to_string(),
                severity: KillSeverity::Mandatory,
                context: HashMap::new(),
            });
        ...
        if confidence.score < self.config.min_overall_confidence {
            ...
            context.insert(
                "required_confidence".to_string(),
                self.config.min_overall_confidence.to_string(),
            );
            ...
            conditions.push(KillCondition {
                ...
                description: format!(
                    ...
                    confidence.score, self.config.min_overall_confidence
                ),
                severity: KillSeverity::Mandatory,
                context,
            });
        ...
        if confidence.components.data_quality < self.config.min_data_quality {
            ...
            context.insert(
                ...
                confidence.components.data_quality.to_string(),
            );
            context.insert(
                "required_quality".to_string(),
                self.config.min_data_quality.to_string(),
            );
            ...
            conditions.push(KillCondition {
                ...
                description: format!(
                    ...
                    confidence.components.data_quality, self.config.min_data_quality
                ),
                severity: KillSeverity::Mandatory,
                context,
            });
...
mod tests {
    ...
    use crate::confidence::types::{
        ConfidenceComponents, ConfidenceLevel, StabilityComponents, StabilityLevel,
    };
    ...
    fn test_proceed_with_valid_inputs() {
        ...
        let decision =
            evaluator.evaluate(&data_quality, &regime, &structural, &confidence, &stability);
    ...
    fn test_halt_on_data_integrity_failure() {
        ...
        assert!(conditions
            .iter()
            .any(|c| c.condition_type == KillConditionType::DataIntegrityFailure));
    }
    ...
    fn test_halt_on_schema_mismatch() {
        ...
        assert!(conditions
            .iter()
            .any(|c| c.condition_type == KillConditionType::UnresolvableSchemaMismatch));
    }
    ...
    fn test_halt_on_missing_core_data() {
        ...
        assert!(conditions
            .iter()
            .any(|c| c.condition_type == KillConditionType::MissingCoreData));
    }
    ...
    fn test_halt_on_time_ambiguity() {
        ...
        assert!(conditions
            .iter()
            .any(|c| c.condition_type == KillConditionType::TimeSemanticViolation));
    }
    ...
    fn test_halt_on_event_ordering_ambiguity() {
        ...
        assert!(conditions
            .iter()
            .any(|c| c.condition_type == KillConditionType::EventOrderingAmbiguity));
    }
    ...
    fn test_halt_on_regime_indeterminacy() {
        ...
        assert!(conditions
            .iter()
            .any(|c| c.condition_type == KillConditionType::RegimeIndeterminacy));
    }
    ...
    fn test_halt_on_conflicting_regime_signals() {
        ...
        regime.conflicts = vec![
            "Bull vs Bear".to_string(),
            "High vol vs Low vol".to_string(),
        ];
        ...
        assert!(conditions
            .iter()
            .any(|c| c.condition_type == KillConditionType::ConflictingRegimeSignals));
    }
    ...
    fn test_halt_on_invalid_constraints() {
        ...
        assert!(conditions
            .iter()
            .any(|c| c.condition_type == KillConditionType::ConstraintMisidentification));
    }
    ...
    fn test_halt_on_forced_flow_contradiction() {
        ...
        assert!(conditions
            .iter()
            .any(|c| c.condition_type == KillConditionType::ForcedFlowContradiction));
    }
    ...
    fn test_halt_on_low_confidence() {
        ...
        assert!(conditions
            .iter()
            .any(|c| c.condition_type == KillConditionType::ConfidenceSystemFailure));
    }
