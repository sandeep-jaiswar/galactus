...
from validation.rules import (EventTimeValidator,  # noqa: E402
                              ForbiddenPracticesDetector, LogicFreezeValidator,
                              RegimeCoverageValidator, ViolationType)
...
class TestRegimeCoverageValidator(unittest.TestCase):
    ...
    def test_insufficient_liquidity_coverage(self):
        ...
        violations = validator.validate_coverage(window_regimes)
        self.assertTrue(
            any(v.violation_type == ViolationType.WINDOW_EXCLUSION for v in violations)
        )
...
class TestForbiddenPracticesDetector(unittest.TestCase):
    ...
    def test_window_exclusion_detection(self):
        ...
        self.assertTrue(
            all(v.violation_type == ViolationType.WINDOW_EXCLUSION for v in violations)
        )
    ...
    def test_high_exclusion_rate_warning(self):
        ...
        self.assertTrue(
            any("exclusion rate" in v.description.lower() for v in violations)
        )
    ...
    def test_retroactive_reclassification_detection(self):
        ...
        self.assertIsNotNone(violation)
        self.assertEqual(
            violation.violation_type, ViolationType.REGIME_RECLASSIFICATION
        )
    ...
    def test_result_smoothing_detection(self):
        ...
        self.assertTrue(
            any(v.violation_type == ViolationType.RESULT_SMOOTHING for v in violations)
        )
    ...
    def test_result_count_mismatch(self):
        ...
        self.assertGreater(len(violations), 0)
        self.assertTrue(
            any("count mismatch" in v.description.lower() for v in violations)
        )
