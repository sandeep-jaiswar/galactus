...
def prevent_production_use(feature_name: str) -> None:
    ...
    if os.getenv("GALACTUS_ENV") == "production":
        raise FeatureIsolationError(
            f"Feature '{feature_name}' is experimental and cannot be used in production. "
            "Features must pass the promotion checklist and be implemented in core/rust "
            "before production use. See: docs/06-research-framework/promotion-checklist.md"
        )
