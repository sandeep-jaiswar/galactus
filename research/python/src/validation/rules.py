        ...
        if access_timestamp < required_availability:
            return Violation(
                ...
                context={
                    ...
                    "gap_hours": (
                        access_timestamp - required_availability
                    ).total_seconds()
                    / 3600,
                },
            )
                ...
                if curr_time < prev_time:
                    violations.append(
                        Violation(
                            violation_type=ViolationType.LOOK_AHEAD_BIAS,
                            description=(
                                f"Inference at {curr_time} uses "
                                f"data from {prev_time}"
                            ),
                            timestamp=datetime.now(),
                            severity="error",
...
class LogicFreezeValidator:
    ...
    def __init__(self, frozen_logic: Any):
        """
            ...
            if len(liquidity_regimes) < 2:
                violations.append(
                    Violation(
                        ...
                        description=(
                            "Insufficient liquidity regime coverage - "
                            "need multiple regimes"
                        ),
                        timestamp=datetime.now(),
                        severity="error",
                        context={
                            "found_regimes": list(liquidity_regimes),
                            "required": 2,
                        },
                    )
                )
            ...
            if len(volatility_regimes) < 2:
                violations.append(
                    Violation(
                        violation_type=ViolationType.WINDOW_EXCLUSION,
                        description=(
                            "Insufficient volatility regime coverage - "
                            "need multiple regimes"
                        ),
                        timestamp=datetime.now(),
                        severity="error",
                        context={
                            "found_regimes": list(volatility_regimes),
                            "required": 2,
                        },
                    )
                )
            ...
            if not has_transition:
                violations.append(
                    Violation(
                        violation_type=ViolationType.WINDOW_EXCLUSION,
                        description=(
                            "No regime transition window " "included in validation"
                        ),
                        timestamp=datetime.now(),
                        severity="error",
                        context={"all_regimes": list(all_regimes)},
...
class ForbiddenPracticesDetector:
        ...
        if exclusion_rate > 0.2:  # More than 20% excluded
            violations.append(
                Violation(
                    violation_type=ViolationType.WINDOW_EXCLUSION,
                    description=(
                        f"High exclusion rate: {exclusion_rate:.1%} "
                        f"of windows excluded"
                    ),
                    timestamp=datetime.now(),
                    severity="warning",
                    context={
    ...
    def detect_retroactive_reclassification(
        self, window_index: int, original_regimes: List[str], new_regimes: List[str]
    ) -> Optional[Violation]:
        """
        ...
        if len(window_results) != len(reported_results):
            violations.append(
                Violation(
                    ...
                    description=(
                        "Result count mismatch - possible aggregation " "or omission"
                    ),
                    timestamp=datetime.now(),
                    severity="error",
                    context={
            ...
            if abs(raw - reported) > tolerance:
                violations.append(
                    Violation(
                        violation_type=ViolationType.RESULT_SMOOTHING,
                        description=(
                            f"Window {i} result modified. Raw: {raw}, "
                            f"Reported: {reported}"
                        ),
                        timestamp=datetime.now(),
                        severity="error",
    ...
    def detect_retroactive_fix(
        self, fix_description: str, fix_timestamp: datetime, validation_start: datetime
    ) -> Optional[Violation]:
        """
        ...
        if fix_timestamp > validation_start:
            violation = Violation(
                ...
                description=(
                    f"Retroactive fix applied during validation: " f"{fix_description}"
                ),
                timestamp=datetime.now(),
                severity="error",
                context={
