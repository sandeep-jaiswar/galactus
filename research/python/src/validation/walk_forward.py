...
class WalkForwardValidator:
    ...
    def set_training_window(
        self, start_time: datetime, end_time: datetime, justification: str
    ) -> None:
        """
        ...
        for val_window in self._validation_windows:
            if window.overlaps_with(val_window):
                raise ValidationError(
                    (
                        "Training window overlaps with validation window - "
                        "this violates temporal honesty"
                    )
                )

        self._training_window = window

    def add_validation_window(
        self,
        start_time: datetime,
        end_time: datetime,
        ...
        if self._training_window and start_time < self._training_window.end_time:
            raise ValidationError(
                (
                    "Validation window starts before training window ends - "
                    "this violates temporal honesty"
                )
            )
        ...
        if self._validation_windows:
            ...
            if window.start_time < last_window.start_time:
                raise ValidationError(
                    "Validation windows must be added in chronological order"
                )
    ...
    def evaluate_window(
        self,
        window_index: int,
        inference_func: Callable[[datetime, datetime], Dict[str, Any]],
    ) -> WindowResult:
        """
    ...
    def finalize(self, documentation: Optional[str] = None) -> ValidationResult:
        ...
        if self._config.require_multiple_liquidity_regimes:
            liquidity_regimes = [
                r for r in regime_coverage.keys() if "liquidity" in r.lower()
            ]
        ...
        if self._config.require_multiple_volatility_regimes:
            volatility_regimes = [
                r for r in regime_coverage.keys() if "volatility" in r.lower()
            ]
        ...
        if self._config.require_expiry_period:
            if not any("expiry" in r.lower() for r in regime_coverage.keys()):
                failures.append(
                    "Validation must include at least one expiry-heavy period"
                )

        if self._config.require_regime_transition:
            if not any("transition" in r.lower() for r in regime_coverage.keys()):
                failures.append(
                    "Validation must include at least one regime transition window"
                )
