...
from pyspark.sql.functions import col, concat_ws, to_date, lit, trim
...
def process_bhavcopy_to_silver(
    spark: SparkSession, csv_content: str, trade_date: str
) -> Optional[DataFrame]:
    """
    Process Bronze bhavcopy CSV content into Silver layer DataFrame

    This function:
    1. Reads raw CSV content with explicit schema
    ...
    """
    logger.info(f"Processing CSV content for trade date: {trade_date}")

    try:
        # Read CSV from content with explicit schema
        schema = get_bhavcopy_schema()
        # Split CSV content by lines for proper parsing
        ...
        df = (
            ...
            .schema(schema)
            .csv(spark.sparkContext.parallelize(csv_lines))
        )

        input_count = df.count()
        logger.info(f"Read {input_count} rows from CSV content")

        if input_count == 0:
            logger.warning("No data in CSV content")
            return None
        ...
        logger.info(
            f"Processed {output_count} valid rows (rejected {input_count - output_count})"
        )
...
def _validate_and_clean(df: DataFrame, expected_date: str) -> DataFrame:
    ...
    for price_col in [
        ...
        df_clean = df_clean.filter(col(price_col).isNull() | (col(price_col) > 0))

    # Validate quantities and trades are non-negative
    for qty_col in ["TTL_TRD_QNTY", "NO_OF_TRADES", "DELIV_QTY"]:
        df_clean = df_clean.filter(col(qty_col).isNull() | (col(qty_col) >= 0))
...
def _add_hudi_columns(df: DataFrame, trade_date: str) -> DataFrame:
    ...
    df = df.withColumn(
        "record_key", concat_ws("-", col("SYMBOL"), col("SERIES"), col("trade_date"))
    )
...
def validate_silver_write(df: DataFrame, trade_date: str) -> bool:
    ...
    if total_records != unique_keys:
        ...
        logger.warning(
            f"Found {duplicates} duplicate record keys - Hudi will handle via upsert"
        )
